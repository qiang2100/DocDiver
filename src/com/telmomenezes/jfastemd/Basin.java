...
class Edge0 {
    Edge0(int to, double cost, double flow) {
        _to = to;
        _cost = cost;
        _flow = flow;
    }
...
class Edge1 {
    Edge1(int to, double reduced_cost) {
        _to = to;
...
class Edge2 {
    Edge2(int to, double reduced_cost, double residual_capacity) {
        _to = to;
        _reduced_cost = reduced_cost;
...
class MinCostFlow {
    ...
    double compute(Vector<Double> e, Vector<List<Edge>> c, Vector<List<Edge0>> x) {
        ...
        double delta =  (Math.pow(2.0,
                Math.ceil(Math.log(U) / Math.log(2.0))));
    ...
    void computeShortestPath(Vector<Double> d, Vector<Integer> prev,
            int from, Vector<List<Edge1>> costForward,
            Vector<List<Edge2>> costBackward, Vector<Double> e, int[] l) {
        ...
        for (int i = 0; i < from; ++i) {
            Q.get(j)._to = i;
            nodesToQ.set(i, j);
            Q.get(j)._dist = Double.MAX_VALUE;
            j++;
        }
        ...
        do {
            ...
            for (Edge1 it : costForward.get(u)) {
                ...
                double alt = d.get(u) + it._reduced_cost;
                int v = it._to;
